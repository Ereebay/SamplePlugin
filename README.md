...
It is the upstream `SamplePlugin` (copied verbatim from AssettoServer **`v0.0.54`**), extracted into a standalone repository so you can click **Use this template** on GitHub and start from a clean slate. Out of the box it demonstrates the five things almost every plugin needs:
...
2. **Clone the AssettoServer host at the version this template targets (`v0.0.54`):**
   ```bash
...
4. **Build:**
   ...
   ```
   The plugin DLL lands in `bin/Release/net8.0/SamplePlugin.dll`.
...
| `Sample.cs` | A `CriticalBackgroundService` + `IAssettoServerAutostart` — runs once the server is fully initialized; an unhandled error stops the host instead of dying silently. |
...
This template targets AssettoServer **`v0.0.54`** (`net8.0`). To build against another release, check out a different tag in the host clone:

```bash
cd AssettoServer && git checkout <tag>
```

> ⚠️ AssettoServer `0.0.55`+ (and `master`) moved to **`net9.0`** and changed the plugin lifecycle API (`CriticalBackgroundService` / `IAssettoServerAutostart` → plain `BackgroundService` / `IHostedService`). If you target one of those, bump `<TargetFramework>` to `net9.0` in `SamplePlugin.csproj` and update `Sample.cs` / `SampleModule.cs` accordingly.

## Continuous integration

`.github/workflows/build.yml` checks out `compujuckel/AssettoServer@v0.0.54`, drops this repo inside it, and builds — so the template is verified end to end on every push. Change the `ref:` (and the framework, per the note above) to target a different server version.
